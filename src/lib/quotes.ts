...
// export async function searchQuotesByEmotion(query: string, limit = 24, page = 1): Promise<QuoteItem[]> {
//   const q = (query ?? "").trim();

//   // Preferred: RPC FTS (make sure you’ve created the function with args: search_text, page, page_size)
//   const { data: rpc, error: rpcErr } = await supabase.rpc("search_quotes_by_emotion", {
//     emotion_filter: q || null,
//     page: 1,
//     page_size: Math.min(limit, 50),
//   });
...
export async function searchQuotesByEmotion(emotion: string, pageSize = 24, page = 1) {
  ...
  if (error) {
    console.error("search_quotes_by_emotion error", error);
    return [];
  }
  return data || [];
}
